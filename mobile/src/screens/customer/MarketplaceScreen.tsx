...
import { isDemoProduct } from '../../config/demo';
...
const ProductCardView = React.memo(({ item, onPress }: { item: any; onPress: () => void }) => {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${item.title || t('untitledCraft')}, ₹${price ?? '—'}`}
      onPress={onPress}
      style={({ pressed }) => [styles.productCardWrap, pressed && styles.productCardPressed]}
    >
      <Card padding={0} style={styles.productCard}>
        ...
        <View style={styles.productBody}>
          ...
          <View style={styles.viewDetailsRow}>
            <Text style={styles.viewDetailsText}>{t('viewDetailsBtn')}</Text>
            <Ionicons name="arrow-forward" size={14} color={PALETTE.primaryLight} />
          </View>
        </View>
      </Card>
    </Pressable>
...
export const MarketplaceScreen: React.FC<{ navigation?: any }> = ({ navigation }) => {
  ...
  const loadMarketplace = async (refresh = false) => {
    ...
    try {
      ...
      setProducts(data.filter((p: any) => !isDemoProduct(p)));
    } catch (err: any) {
