...
import { optionalAuth, provisionRole, requireAuth, requireRole, samePhone } from './serverAuth.ts';
...
function resolveMediaReference(value: unknown, auth?: Express.Request['auth']): string {
...
function hydrateProductMedia(product: any, auth?: Express.Request['auth']): any {
  return {
    ...product,
...
function visibleProductsFor(auth?: Express.Request['auth']): any[] {
  const products = DEMO_DATA_ENABLED ? productsDb : productsDb.filter((product) => !isDemoProduct(product));
  if (!auth) {
    return products
      .filter((product) => product.status === 'PUBLISHED')
      .map(({ artisanId, artisanPhone, ...product }) => hydrateProductMedia(product, undefined));
  }
  ...
  return products
    .filter((product) => product.status === 'PUBLISHED')
    .map(({ artisanId, artisanPhone, ...product }) => hydrateProductMedia(product, auth));
}
...
function validateStoredFileAccess(fileId: string, auth?: Express.Request['auth']) {
  const file = filesDb.find((item) => item.id === fileId);
  if (!file) return null;
  if (!auth) {
    const product = productsDb.find((item) => item.id === file.productId);
    if (product?.status === 'PUBLISHED') return file;
    return null;
  }
...
// Public or optionally-authenticated read routes
app.get('/api/products', authRateLimit, optionalAuth, (req, res) => {
  res.json(visibleProductsFor(req.auth));
});

app.get('/api/files/:id', authRateLimit, optionalAuth, validateParam('id', /^[0-9a-f-]{36}$/), (req, res) => {
...
// All application data mutations, AI, profile, and inquiries require a verified identity.
